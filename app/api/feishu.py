...
from app.domain.order_state import InvalidTransition, StaleOrderVersion, assert_version
...
def _persist_callback_response(
    db: Session,
    event_id: str,
    nonce: str,
    status_code: int,
    ...
    if callback is None:
        ...
        try:
            ...
            db.rollback()
            callback = db.scalar(
                select(FeishuCallback).where(FeishuCallback.event_id == event_id)
            )
            if callback is None:
                raise
